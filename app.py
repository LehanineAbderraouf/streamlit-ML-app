...
def page3():
	...
	df_full = pd.DataFrame(pd.concat([pd.DataFrame(pd.concat([X_train,X_valid], axis=0)).reset_index(),pd.DataFrame(pd.concat([y_train,y_valid], axis=0)).reset_index()],axis=1))
	df = pd.DataFrame(pd.concat([X_train,X_valid], axis=0))

	st.header("Choose the Feature:")
	selected_feature=st.selectbox(" ",tuple(df.columns),label_visibility="collapsed")


	
	
	st.header("Unidimentional Data Analysis")
	
	fig, ax = plt.subplots()
	plt.hist(df[selected_feature])
	plt.legend(loc=4)
	plt.xlabel(selected_feature+" values")
	plt.ylabel("count")
	st.pyplot(fig)

	st.header("Bidimentional Data Analysis with the target")

	target_unique_values = df_full[y_train.columns[0]].unique()
	df_by_target = [ df_full[df_full[y_train.columns[0]]==i][selected_feature] for i in target_unique_values]
	fig = ff.create_distplot(df_by_target, [str(num) for num in target_unique_values])
...
def page4():
	...
	st.header("Evaluation with Precision, Recall, F-score and Support:")
...
page_names_to_funcs = {
	...
	"Data Visualisation": page3,
	"Machine Learning": page4,
}
